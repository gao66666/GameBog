...
class BlogClient:
    ...
    def _request(self, url: str, method: str = "GET", body: dict | None = None, token: str = "") -> dict:
        ...
        try:
            ...
            _log.error(
                "HTTPError status=%s url=%s body=%s",
                e.code,
                url,
                err_body[:200],
            )
    ...
    def list_games(self, page: int = 1, size: int = 100) -> dict:
        return self._get("/games", {"page": str(page), "size": str(size)})

    def search_games(self, q: str, limit: int = 20) -> dict:
        return self._get("/games/search", {"q": q, "limit": str(limit)})
    ...
    def list_points_mall_products(self) -> dict:
        return self._get("/points/mall/products")

    def search_points_mall_products(self, q: str, limit: int = 20) -> dict:
        return self._get("/points/mall/products/search", {"q": q, "limit": str(limit)})

    def get_points_mall_product(self, product_id: int) -> dict:
        return self._get(f"/points/mall/products/{product_id}")
