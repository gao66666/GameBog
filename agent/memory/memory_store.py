...
async def get_all_short_messages(user_id: int, session_id: str | None = None) -> list[dict]:
    """读取某会话桶内全部短期消息（不限条数）。"""
    if user_id <= 0:
        return []
    key = _short_list_key(user_id, session_id)
    raw = await _r().lrange(key, 0, -1)
    messages: list[dict] = []
    for item in raw:
        try:
            msg = json.loads(item)
...
async def list_short_session_ids(user_id: int) -> list[str]:
    """列出用户所有带短期 list 的 chat_session_id（去重）。"""
    if user_id <= 0:
        return []
    prefix = f"agent:short:{user_id}:"
    ids: list[str] = []
    seen: set[str] = set()
    async for key in _r().scan_iter(match=f"agent:short:{user_id}:*:list"):
        if not key.startswith(prefix) or not key.endswith(":list"):
            continue
        sid = key[len(prefix) : -len(":list")]
...
async def delete_short_session(user_id: int, session_id: str | None = None) -> None:
    """删除某会话短期 list + state（中期 flush 或会话清理后）。"""
    if user_id <= 0:
        return
    pipe = _r().pipeline()
    pipe.delete(_short_list_key(user_id, session_id))
    pipe.delete(_short_state_key(user_id, session_id))
    await pipe.execute()
...
async def update_mid_summary(user_id: int, prev_summary: dict | None, history_messages: list[dict]) -> dict | None:
    ...
    summary_cfg = {**cfg, **(cfg.get("summary_llm") or {})}
    llm = ChatOpenAI(**chat_openai_kwargs(summary_cfg, temperature=0))

    fc_max = int(mem_cfg.get("mid_facts_max_chars", 560))
    fi_max = int(mem_cfg.get("mid_facts_max_items", 10))

    prompts = get_prompts()
    system = prompts["mid_summary_system"].format(
        mid_facts_max_items=fi_max,
        mid_facts_max_chars=fc_max,
    )

    prev = json.dumps(prev_summary or {}, ensure_ascii=False)
    history_json = json.dumps(history_messages, ensure_ascii=False)
    prompt = prompts["mid_summary_user"].format(
        utc_now=_utc_now_z(),
        prev_summary_json=prev,
