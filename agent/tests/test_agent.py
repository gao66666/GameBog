"""
Agent 手工联调 — 直接调 /chat 并打印 SSE 事件

用法:
  python tests/test_agent.py "有什么热门文章"
  python tests/test_agent.py "我的积分多少" --token=xxx
"""
...
AGENT_URL = os.getenv("AGENT_URL", "http://127.0.0.1:9091").rstrip("/") + "/chat"
    ...
    with urllib.request.urlopen(req, timeout=120) as resp:
        for line in resp:
            ...
            if t == "rewrite":
                ...
                print(
                    f"[路由] servers={data['servers']} tool_hints={data.get('tool_hints', [])} "
                    f"memory_hints={data.get('memory_hints', [])} ({data['route_ms']}ms)"
                )
            elif t == "token":
